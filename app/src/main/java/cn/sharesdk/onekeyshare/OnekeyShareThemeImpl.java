...
public abstract class OnekeyShareThemeImpl implements PlatformActionListener, Callback {
	...
	private void postAsynHttp() {
		...
		RequestBody formBody = new FormBody.Builder()
				.add("member_id", BPApplication.getInstance().getMember_Id())
				.add("secret", CreateMD5.getMd5(BPApplication.getInstance().getMember_Id()+ SystemConstant.PublicConstant.Public_SECRET))
				.build();
		Request request = new Request.Builder()
				.url(SystemConstant.GAME.Game_API_POINT)
				.post(formBody)
				.build();
